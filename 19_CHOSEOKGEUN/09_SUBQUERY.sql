...
SELECT -- 메인쿼리
       category_name
  FROM tbl_category
 WHERE category_code = (SELECT category_code  -- 서브 퀘리
                           FROM tbl_menu
...
SELECT
  ...
  FROM
    tbl_menu
 WHERE category_code = (SELECT category_code  -- WHERE 절 와 서브 퀘리의 값이 동일 해야함
                          FROM tbl_menu
...
-- 서브퀘리가 다중행을 반환하는 경우 = 연사자를 사용할 수 없다.
SELECT
        *
  FROM
       tbl_menu
...
SELECT
        category_code, MAX(menu_price) -- 그룹함수
  FROM tbl_menu
